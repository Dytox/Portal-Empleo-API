...
export const getAllCompanyProfiles = async () => {
  try {
    const result = await pool.query('SELECT * FROM public.company_profile');
    return result.rows;
  } catch (err) {
...
export const getCompanyProfileById = async (id) => {
  try {
    const result = await pool.query('SELECT * FROM public.company_profile WHERE id = $1', [id]);
...
export const getCompanyProfileByUserId = async (userId) => {
  try {
    const result = await pool.query('SELECT * FROM public.company_profile WHERE user_id = $1', [userId]);
    if (result.rows.length === 0) {
      return null;
    }
    return result.rows[0];
...
export const createCompanyProfile = async (profileData) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO public.company_profile (user_id, company_name, tel, external_links_id, additional_info_id) VALUES ($1, $2, $3, $4, $5) RETURNING *',
...
export const updateCompanyProfile = async (id, profileData) => {
  try {
    ...
    const query = `UPDATE public.company_profile SET ${updates.join(', ')} WHERE id = $${paramCount} RETURNING *`;
...
export const deleteCompanyProfile = async (id) => {
  try {
    const result = await pool.query('DELETE FROM public.company_profile WHERE id = $1 RETURNING *', [id]);
    if (result.rows.length === 0) {
      return null;
    }
    return result.rows[0];
