...
export const getAllCompanySizes = async () => {
  try {
    const result = await pool.query('SELECT * FROM public.company_size');
    return result.rows;
  } catch (err) {
...
export const getCompanySizeById = async (id) => {
  try {
    const result = await pool.query('SELECT * FROM public.company_size WHERE id = $1', [id]);
...
export const updateCompanySize = async (id, sizeData) => {
  try {
    ...
    const result = await pool.query(
      'UPDATE public.company_size SET size_name = $1 WHERE id = $2 RETURNING *',
      [size_name, id]
    );
    if (result.rows.length === 0) {
      return null;
...
export const deleteCompanySize = async (id) => {
  try {
    const result = await pool.query('DELETE FROM public.company_size WHERE id = $1 RETURNING *', [id]);
    if (result.rows.length === 0) {
      return null;
    }
    return result.rows[0];
