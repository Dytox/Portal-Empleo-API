...
export const getAllProfileSkills = async () => {
  try {
    const result = await pool.query('SELECT * FROM public.user_profile_skills');
    return result.rows;
  } catch (err) {
    throw new Error(`Database error: ${err.message}`);
  }
...
export const getProfileSkillById = async (id) => {
  try {
    const result = await pool.query('SELECT * FROM public.user_profile_skills WHERE id = $1', [id]);
    if (result.rows.length === 0) {
      return null;
    }
    return result.rows[0];
...
export const getProfileSkillsByProfileId = async (profileId) => {
  try {
    const result = await pool.query('SELECT * FROM public.user_profile_skills WHERE profile_id = $1', [profileId]);
    return result.rows;
  } catch (err) {
    throw new Error(`Database error: ${err.message}`);
  }
...
export const createProfileSkill = async (profileSkillData) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO public.user_profile_skills (profile_id, skill_id) VALUES ($1, $2) RETURNING *',
      [profile_id, skill_id]
    );
    return result.rows[0];
  } catch (err) {
...
export const deleteProfileSkill = async (id) => {
  try {
    const result = await pool.query('DELETE FROM public.user_profile_skills WHERE id = $1 RETURNING *', [id]);
    if (result.rows.length === 0) {
      return null;
    }
    return result.rows[0];
